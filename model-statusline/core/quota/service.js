...
function hasPastResetTime(cached, now) {
  if (!cached?.result?.quotas) return false;
  for (const q of cached.result.quotas) {
    if (q.nextResetTime && q.nextResetTime < now) return true;
  }
  if (cached.result.nextResetTime && cached.result.nextResetTime < now) return true;
  return false;
}
...
function shouldRefreshQuota(cached, cacheTtlMs, now, sessionId) {
  ...
  // If any reset time is in the past, force refresh since quota has reset
  if (hasPastResetTime(cached, now)) {
    return true;
  }
...
/**
 * @param {object} config - quota config object (must contain provider)
...
export async function resolveQuotaStatus(config, options = {}) {
  ...
  const response = await config.provider.fetch(config, fetchImpl);
  const parsed = config.provider.parse(response);
