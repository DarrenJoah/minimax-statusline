...
function formatLevel(level, displayName) {
  if (!level) {
    return displayName;
  }

  return `${displayName} ${level.charAt(0).toUpperCase()}${level.slice(1)}`;
}
...
export function formatQueryHuman(result, displayMode, displayName = "GLM") {
  if (!result || typeof result !== "object") {
    return `${displayName} | quota unavailable\n`;
  }

  if (result.kind === "auth_error") {
    return `${displayName} | auth expired\n`;
  }
  ...
  // Balance-based display (Kimi)
  if (result.display === "balance" && Number.isFinite(result.balance)) {
    const lines = [
      `${displayName} Balance`,
      "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━",
      `  balance:   ¥${result.balanceText}`
    ];
    return `${lines.join("\n")}\n`;
  }

  const mode = normalizeDisplayMode(displayMode);
  const levelLabel = formatLevel(result.level, displayName);
  const lines = [`${levelLabel} Statusline`, "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"];
  ...
  if (
    result.mcp &&
    Number.isFinite(mode === "used" ? result.mcp.usedPercent : result.mcp.leftPercent)
  ) {
