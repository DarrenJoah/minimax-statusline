...
function normalizeQuota(quota) {
  ...
  // Balance-based quota (Kimi: monetary balance in CNY)
  if (quota.balance !== undefined) {
    return {
      key: quota.key || "balance",
      label: "balance",
      compactLabel: "balance",
      leftPercent: null,
      usedPercent: null,
      leftText: `¥${quota.balanceText}`,
      usedText: `¥${quota.balanceText}`,
  ...
  if (!Number.isFinite(quota.leftPercent)) {
    return null;
  }
...
export function buildStatusViewModel(result, options = {}) {
  ...
  const compactLabel = options.compactLabel || "GLM";
  ...
  // Balance-based view (Kimi: show monetary balance)
  if (primaryQuota.balance !== undefined) {
    return {
      kind: "success",
  ...
  return {
    kind: "success",
    levelLabel: formatLevel(result.level, displayName),
    compactLabel,
    primaryQuota,
    secondaryQuota,
