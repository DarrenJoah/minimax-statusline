...
import { isKnownProvider, ALL_PROVIDERS } from "../core/providers/index.js";
...
export function getToolConfigPath() {
  return path.join(getClaudeDir(), "model-statusline.json");
}
...
function normalizeToolConfig(config) {
  ...
  if (isKnownProvider(base.provider)) {
    normalized.provider = base.provider;
  }

  // Per-provider auth tokens (e.g. authToken_minimax-intl)
  for (const p of ALL_PROVIDERS) {
    const key = `authToken_${p.name}`;
    const val = normalizeOptionalString(base[key]);
    if (val) {
      normalized[key] = val;
    }
    const urlKey = `baseUrl_${p.name}`;
    const urlVal = normalizeOptionalString(base[urlKey]);
    if (urlVal) {
      normalized[urlKey] = urlVal;
    }
  }
